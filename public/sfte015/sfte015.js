...
$(function(){
	...
	try { startApplication("sfte015"); }catch(ex) { console.warn("error",ex);  }
	initialApplication();
...
function search(aform) {
	...
	startWaiting();
	jQuery.ajax({
		url: getApiUrl()+"/api/sfte015/search",
...
function insert() {
	...
	jQuery.ajax({
		...
		success: function(data,status,transport){
			...
			$("#fsmodaldialog_layer").on("shown.bs.modal",function() {
				$("#msgcode").trigger("focus");
			}).modal("show");
		}
	});
	return false;
...
function saveForm(aform) {
	...
	startWaiting();
	jQuery.ajax({
		url: getApiUrl()+"/api/sfte015/insert",
		data: formdata.jsondata,
		headers : formdata.headers,
		type: "POST",
		dataType: "html",
		...
		success: function(data,status,transport){
			...
			successbox(function() { 
				aform.reset();
				setTimeout(function() { $("#msgcode").focus(); },200);
			});
...
function updateForm(aform) {
	...
	startWaiting();
	jQuery.ajax({
		url: getApiUrl()+"/api/sfte015/update",
		data: formdata.jsondata,
		headers : formdata.headers,
		type: "POST",
		dataType: "html",
...
function submitRetrieve(src,msgcode,langcode) {
	//#(250000) programmer code begin;
	//#(250000) programmer code end;
	let aform = fslistform;
	aform.msgcode.value = msgcode;
	aform.langcode.value = langcode;
	let formdata = serializeDataForm(aform);
	startWaiting();
	jQuery.ajax({
		url: getApiUrl()+"/api/sfte015/retrieval",
		data: formdata.jsondata,
		headers: formdata.headers,
		type: "POST",
		dataType: "html",
...
function submitDelete(src,fsParams) {
	...
	confirmDelete([fsParams[0]+"-"+fsParams[2]],function() {
...
function deleteRecord(fsParams) {
	...
	startWaiting();
	jQuery.ajax({
		url: getApiUrl()+"/api/sfte015/remove",
		data: formdata.jsondata,
		headers: formdata.headers,
		type: "POST",
		dataType: "html",
...
function setupDataTable() {
	...
	$("#datatablebody").find(".fa-data-edit").each(function(index,element) {
		$(element).click(function() {
			...
			submitRetrieve(element,$tr.attr("data-key"),$tr.attr("data-lang"));
		});
	});
	$("#datatablebody").find(".fa-data-delete").each(function(index,element) {
		$(element).click(function() {
			if($(this).is(":disabled")) return;
			let $tr = $(this).closest("tr");
			submitDelete(element,[$tr.attr("data-key"),$tr.attr("data-lang"),$(this).attr("data-name")]);
		});
	});
}
