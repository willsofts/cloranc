...
$(function(){
	...
	try { startApplication("sfte010"); }catch(ex) { console.warn("error",ex);  }
	initialApplication();
...
function search(aform) {
	...
	startWaiting();
	jQuery.ajax({
		url: getApiUrl()+"/api/sfte010/search",
...
function validSaveForm(callback) {
	...
	return validRequiredFields(callback,getRequiredFields());
	//#(180000) programmer code begin;
	//#(180000) programmer code end;
}
function save(aform) {
	//#(190000) programmer code begin;
	setRequiredFields({ "reservepwd":{msg:""} });
...
function saveForm(aform) {
	...
	startWaiting();
	jQuery.ajax({
		url: getApiUrl()+"/api/sfte010/insert",
		data: formdata.jsondata,
		headers : formdata.headers,
		type: "POST",
		dataType: "html",
...
function updateForm(aform) {
	...
	startWaiting();
	jQuery.ajax({
		url: getApiUrl()+"/api/sfte010/update",
		data: formdata.jsondata,
		headers : formdata.headers,
		type: "POST",
		dataType: "html",
...
function submitRetrieve(src,reservepwd) {
	...
	startWaiting();
	jQuery.ajax({
		url: getApiUrl()+"/api/sfte010/retrieval",
		data: formdata.jsondata,
		headers: formdata.headers,
		type: "POST",
		dataType: "html",
...
function deleteRecord(fsParams) {
	...
	startWaiting();
	jQuery.ajax({
		url: getApiUrl()+"/api/sfte010/remove",
		data: formdata.jsondata,
		headers: formdata.headers,
		type: "POST",
		dataType: "html",
...
function getRequiredFields() { return fs_requiredfields; }
function setRequiredFields(values) { fs_requiredfields = values; }
