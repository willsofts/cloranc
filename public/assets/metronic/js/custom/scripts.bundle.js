		...
		var Plugin = {
			...
			adjustCellsWidth: function() {
				...
				var columns = $(datatable.tableBody).
					...
					not(':hidden').length;
 					//sophon_soe add edit bug
                var columns2 = $(datatable.tableBody).
                    find('.' + pfx + 'datatable__row:first-child').
                    find('.' + pfx + 'datatable__cell').not('.' + pfx + 'datatable__toggle-detail').length;
...
//not(':hidden').each(function(tdi, td) { //sophon_soe comment edit bug
				...
				var pg = {
					...
					populate: function() {
						...
						if (end > pg.meta.pages) {
						 ...
						 var btnStart = start + 1;//sophon_soe add
                        var btnEnd = (end || 1)//sophon_soe add

						for (var x = start; x < end; x++) {
							var pageNumber = x + 1;
							if ((btnStart > (pagesNumber - 1)) && (pg.meta.total > pagesNumber) && (pageNumber == btnStart || pageNumber == btnEnd)) {//sophon_soe add
  
                                if (pageNumber == pg.meta.pages) {
                                    $('<li/>').
									append($('<a/>').
									addClass(pfx + 'datatable__pager-link ' + pfx + 'datatable__pager-link-number').
									text(pageNumber).
									attr('onClick', 'javascript:scrollwindows();').
									attr('data-page', pageNumber).
									attr('title', pageNumber).
									on('click', pg.gotoPage)).
									appendTo(pagerNumber);
                                } else {
									if(pageNumber == btnStart){
											$('<li/>').
											append($('<a/>').
											addClass(pfx + 'datatable__pager-link ' + pfx + 'datatable__pager-link-number').
											text('...').
											on('click', pg.gotoMorePage).
											attr('data-page', btnStart-1)).
											appendTo(pagerNumber);
										}
			   								$('<li/>').
											append($('<a/>').
										...
										if(pageNumber == btnEnd){
											$('<li/>').
											append($('<a/>').
											addClass(pfx + 'datatable__pager-link ' + pfx + 'datatable__pager-link-number').
											text('...').
											on('click', pg.gotoMorePage).
											attr('data-page', btnEnd+1)).
											appendTo(pagerNumber);
										}
                              	
                                }                             
                            } else {
                                if (pg.meta.pages > pagesNumber && start == 0 && pageNumber == btnEnd) {
                                  $('<li/>').
									append($('<a/>').
									addClass(pfx + 'datatable__pager-link ' + pfx + 'datatable__pager-link-number').
									text(pageNumber).
					...
					openPage: function(page) {
						...
						//edit bug task NCIK-68 by sophon_soe
						 $(".bs-container.kt-datatable__pager-size").remove();
					},
	...
	$.fn[pluginName].defaults = {
		...
		data: {
			type: 'local',
			source: null,
			//pageSize: 5, // display records per page so edit for 10 to 5
			saveState: {
		...
		rows: {
			...
			autoHide: false,//sophon_soe fix false Auto hide columns which are overflow in a row for the responsive mode.
		},
			...
			items: {
				...
				pagination: {
					...
					pages: {
						desktop: {
							layout: 'default',
							pagesNumber: 8,//sophon_soe custom 5 to 8
						},
						tablet: {
							layout: 'default',
							pagesNumber: 3,
...
var KTLayout = function() {
	...
	var initAside = function() {
		...
		//	KTUtil.addEvent('#kt_aside_toggler', 'click', function(e) {
				e.preventDefault();
		...
		if (KTUtil.attr(menu, 'data-ktmenu-scroll') === '1') {
			scroll = {
				...
				height: function() { // calculate available scrollable area height
					...
					return height-60;//sophon_soe edit bug menu  add -60
				}
			};
		}
...
var KTQuickSearch = function() {
    ...
    return {
        init: function(element) {
			...
			//sophon_soe comment onkeypress error  Cannot set property 'onkeypress' of null
